//
//  WXSearchPicturesViewController.m
//  基于IOS的图片认知分类系统
//
//  Created by 王旭 on 2017/5/18.
...
@interface WXSearchPicturesViewController ()<UICollectionViewDelegate, UICollectionViewDataSource, YuriWaterFallLayoutDelegate,FlowCollectionViewCellDelegate>
...
@implementation WXSearchPicturesViewController
...
-(void)PullOnLoading{
    ...
    if ([self.imageUrlString isEqualToString:myFavoritePictures]) {
        
         parameters = [NSDictionary dictionaryWithObjectsAndKeys:@"18131371659",@"nickname",[NSString stringWithFormat:@"%d",self.number],@"index",nil];
    }
    else{
        parameters = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d",self.number],@"index",[NSString stringWithFormat:@"%@",self.title],@"searchValue",nil];
    }
    
    [AFNHttpTool POST:self.imageUrlString parameters:parameters success:^(id responseObject) {
