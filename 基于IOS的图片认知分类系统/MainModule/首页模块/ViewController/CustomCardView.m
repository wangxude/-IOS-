...
- (void)installData:(NSDictionary *)element {
    [self.imageView sd_setImageWithURL:[element[@"picture"] zp_url] placeholderImage:nil];
//    self.imageView.image  = [UIImage imageNamed:element[@"image"]];
    if ([element[@"tag1"]isEqual:[NSNull null]]||[element[@"tag1"] isEqualToString:@""]||element[@"tag1"]==nil) {
        self.titleLabel.text = @"暂无标签";
    }
    else{
         self.titleLabel.text = [NSString stringWithFormat:@"%@",element[@"tag1"]];
         }
