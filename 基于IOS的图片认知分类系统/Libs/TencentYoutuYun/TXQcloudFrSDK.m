...
- (void)imageTagValue:(id)image cookie:(NSString *)cookie seq:(NSString *)seq successBlock:(HttpRequestSuccessBlock)successBlock failureBlock:(HttpRequestFailBlock)failureBlock
{
    NSMutableDictionary *json = [NSMutableDictionary dictionaryWithCapacity:10];
    if ([image isKindOfClass:[UIImage class]]) {
        json[@"image"] = [self imageBase64String:image];
